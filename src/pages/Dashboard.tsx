...
const getRecurringDates = (reminder: Reminder) => {
  const dates: Date[] = [];
  let current = new Date(reminder.due_date);

  for (let i = 0; i < 12; i++) { // show up to 1 year
...
const Dashboard = () => {
  ...
  getRecurringDates(r).some((d) => isSameDay(d, date))
    );
